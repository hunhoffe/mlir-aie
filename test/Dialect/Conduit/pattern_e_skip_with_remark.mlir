// RUN: aie-opt --objectfifo-to-conduit --dma-task-to-conduit --conduit-fuse-operators --split-input-file --verify-diagnostics %s
//
// Task #44 — Pattern E (forward-chain endpoint) fusion-skip remark.
// Companion to `pattern_e_no_fuse_with_neighbor.mlir`: pins the explicit
// `expected-remark` so the diagnostic text is part of the regression net
// and a future refactor cannot silently demote it.
//
// `--conduit-fuse-operators` MUST emit a remark when it rejects a
// fusion_group / element_type match because one side is a forward-chain
// endpoint (referenced by `conduit.scatter` or `conduit.gather`).  The
// remark is anchored on the `conduit.create` (Pass A lowers the original
// `aie.objectfifo` into a `conduit.create`, so `expected-remark@+1`
...
// expected-warning@+1 {{conduit-fuse-operators: no matching channel pair found between device 0 and device 1 by fusion_group or element_type; skipping}}
module @remark_producer_side {
  aie.device(npu2) @devA {
    ...
    // expected-remark@+1 {{conduit-fuse-operators: skipping fusion_group match for output channel @fwd_out — forward-chain / link-only endpoint (Pattern E); scatter/gather references cannot be safely renamed}}
...
module @remark_consumer_side {
  ...
  aie.device(npu2) @devB {
    ...
    // expected-remark@+1 {{conduit-fuse-operators: skipping fusion_group match for input channel @fwd_in_b — forward-chain / link-only endpoint (Pattern E); scatter/gather references cannot be safely renamed}}
