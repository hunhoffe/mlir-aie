...
// FlatSymbolRefAttr occurrences, including conduit.scatter/gather
// src/dsts/srcs/dst attrs.  Pass A must revert these conduit op references
// back to @ch so that Pass C's conduitMap lookup (keyed on conduit.create
// sym_name) succeeds.
//
// Test setup:
...
//       * conduit.scatter src must remain @link_in  (NOT @link_in_shim_alloc)
...
//
// Also covers gather: shim tile is consumer of @join_out, which is the dst
// of a gather.  Phase 4.5 must not rename @join_out in conduit.gather dst.
...
// PASSA:       conduit.scatter{src = @link_in, dsts = [@link_out_a, @link_out_b]
...
// PASSA:       conduit.gather{srcs = [@join_src_a, @join_src_b], dst = @join_out
// PASSA-NOT:   dst = @join_out_shim_alloc
...
// PASSC-NOT:   conduit.scatter
// PASSC-NOT:   conduit.gather
