...
//   - L2 structural match with the oracle (2 flows, 4 locks, 2 buffers, 4 BDs)
//
// Source: test/objectFifo-stateful-transform/aie_stream/stream_to_link_AIE2.mlir

// --- Conduit pipeline checks ---
// Buffers on MemTile:
// CONDUIT-DAG: aie.buffer({{.*}}) {sym_name = "of_stream_cons_buff_0"}
// CONDUIT-DAG: aie.buffer({{.*}}) {sym_name = "of_stream_cons_buff_1"}
// Stream flow uses Core wire bundle; link flow uses DMA wire bundle:
// CONDUIT-DAG: aie.flow({{.*}}, Core : 0, {{.*}}, DMA : 0)
// CONDUIT-DAG: aie.flow({{.*}}, DMA : 0, {{.*}}, DMA : 0)
// MemTile DMA with S2MM and MM2S:
// CONDUIT: aie.memtile_dma
// CONDUIT: aie.dma_start(S2MM
// CONDUIT: aie.dma_start(MM2S
// No residual conduit ops or objectfifos:
// CONDUIT-NOT: conduit.
// CONDUIT-NOT: aie.objectfifo

// --- Oracle checks ---
// ORACLE-DAG: aie.flow({{.*}}, Core : 0, {{.*}}, DMA : 0)
// ORACLE-DAG: aie.flow({{.*}}, DMA : 0, {{.*}}, DMA : 0)
