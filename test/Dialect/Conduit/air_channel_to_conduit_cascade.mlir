...
//
// Also verifies that dynamic offset operands are silently ignored for cascade
// channels (they are irrelevant — only element[0] is loaded/stored).
...
// Dynamic offsets do NOT trigger a hard error for cascade channels.
// (The offset is irrelevant; we always load/store element[0].)
// CHECK: conduit.put_cascade "cas_chan"
...
module {
  ...
  // Dynamic-offset put: should NOT error (cascade ignores offsets/sizes/strides).
  func.func @test_cascade_dynamic(%src : memref<1xvector<16xi32>>, %dyn : index) {
    %c1 = arith.constant 1 : index
