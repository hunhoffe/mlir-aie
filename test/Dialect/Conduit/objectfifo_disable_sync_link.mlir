...
// CHECK:     aie.lock({{.*}}) {init = 1 : i32, sym_name = "link2_prod_lock_0"}
// CHECK:     aie.lock({{.*}}) {init = 0 : i32, sym_name = "link2_cons_lock_0"}
// CHECK:     aie.lock({{.*}}) {init = 1 : i32, sym_name = "link1_prod_lock_0"}
// CHECK:     aie.lock({{.*}}) {init = 0 : i32, sym_name = "link1_cons_lock_0"}
// No locks for link3 (disable_synchronization=true): no join MemTile locks.
// CHECK-NOT: link3_prod_lock
// CHECK-NOT: link3_cons_lock
// Flows and DMA BD chains must still be emitted.
// CHECK:     aie.flow
...
// Consumer mem BDs for link2 and link1 have use_lock.
// CHECK:     aie.mem
// CHECK:       aie.use_lock
// CHECK:     aie.mem
// CHECK:       aie.use_lock
