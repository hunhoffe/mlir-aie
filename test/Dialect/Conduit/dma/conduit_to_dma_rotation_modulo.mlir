...
// Note: the rotation counter is allocated as aie.buffer at device level.

// CHECK-LABEL: module @rotation_modulo_test
// CHECK:   aie.device(npu1_1col) {

// --- Rotation counter allocated as aie.buffer at device level ---
// CHECK:     %[[ROTBUF:.*]] = aie.buffer(%{{.*}}tile_0_2) {sym_name = "rotation_counter_0_2"} : memref<1xi32>
// --- Counter initialized to 0 at device level ---
// CHECK:     memref.store {{.*}} %[[ROTBUF]]{{.*}} : memref<1xi32>
// CHECK:     aie.core(%{{.*}}tile_0_2) {
// CHECK-NOT:   memref.alloca
// CHECK:       scf.for
// --- Counter loaded, used for scf.if chain buffer selection, then incremented ---
// CHECK:         memref.load %[[ROTBUF]]{{.*}} : memref<1xi32>
