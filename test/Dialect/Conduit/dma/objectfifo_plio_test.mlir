...
// Test PLIO objectfifo support: 3 objectfifos with  on xcve2302.
...
//
// Conduit must:
// 1. Emit aie.shim_dma_allocation with {conduit_channel = @of_0}
...
// CHECK: aie.shim_dma_allocation @of_0_shim_alloc(%{{.*}}, MM2S, 0) {conduit_channel = @of_0}
// CHECK: aie.shim_dma_allocation @of_1_shim_alloc(%{{.*}}, S2MM, 0) {conduit_channel = @of_1}
// CHECK: aie.shim_dma_allocation @of_2_shim_alloc(%{{.*}}, S2MM, 1) {conduit_channel = @of_2}

// CHECK: aie.flow(%{{.*}}, DMA : 0, %{{.*}}, DMA : 0)
// CHECK: aie.flow(%{{.*}}, DMA : 0, %{{.*}}, DMA : 0)
// CHECK: aie.flow(%{{.*}}, DMA : 1, %{{.*}}, DMA : 1)
...
module @plio {
    aie.device(xcve2302) {
        ...
        aie.objectfifo @of_0 (%tile20, {%tile22}, 2 : i32) {} : !aie.objectfifo<memref<64xi16>>
        aie.objectfifo @of_1 (%tile22, {%tile20}, 2 : i32) {} : !aie.objectfifo<memref<64xi16>>
        aie.objectfifo @of_2 (%tile22, {%tile20, %tile23}, 2 : i32) {} : !aie.objectfifo<memref<64xi16>>
