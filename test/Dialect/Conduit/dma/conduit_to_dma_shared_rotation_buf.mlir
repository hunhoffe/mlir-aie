...
//   a single shared rotation counter buffer (memref<Nxi32>) on that tile,
...
//   The rotation counter is an aie.buffer at device level.
//
...
//
// Key assertions:
//   - Exactly ONE aie.buffer "rotation_counter_0_2" : memref<2xi32> at device level.
...
// --- ONE shared rotation counter as aie.buffer at device level (memref<2xi32>) ---
// CHECK:     %[[ROTBUF:.*]] = aie.buffer(%{{.*}}tile_0_2) {sym_name = "rotation_counter_0_2"} : memref<2xi32>
// --- Counter init stores at device level ---
// CHECK:     memref.store %c0_i32{{.*}}, %[[ROTBUF]][%c{{[01]}}{{.*}}] : memref<2xi32>
// CHECK:     memref.store %c0_i32{{.*}}, %[[ROTBUF]][%c{{[01]}}{{.*}}] : memref<2xi32>
...
// CHECK:       memref.load %[[ROTBUF]][%c0{{.*}}] : memref<2xi32>
...
// CHECK:       arith.andi {{.*}} %c1_i32{{.*}} : i32
// CHECK:       memref.store {{.*}} %[[ROTBUF]][%c1{{.*}}] : memref<2xi32>
