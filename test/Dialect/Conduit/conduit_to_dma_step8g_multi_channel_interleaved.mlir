...
//   await_in_a,  configure_in_b, start
//   await_out_a, configure_out_b, start
//   await_in_b,  configure_in_c, start
//   await_out_b, configure_out_c, start
//   trailing: free_in_c, await_out_c
// Both MM2S and S2MM use dma_await_task at the per-op release boundary
// (await guarantees prior queued repeat_count fires drain before the
// next configure overwrites the BD register).  BD-pool recycling is
// preserved by AIEAssignRuntimeSequenceBDIDs synthesizing a
// dma_free_task after each await before its interval-collection walk.
// Trailing release for the last invocation on each channel is a
// dma_free_task (in_data MM2S) / dma_await_task (out_data S2MM).
...
// ordered shape): 6 configures (3 per channel), 5 in-loop awaits
// (2 in_data MM2S + 2 out_data S2MM + 1 trailing out_data S2MM),
// 1 trailing free (in_data MM2S last-invocation cleanup).
...
// Second in: await of [[IN0]] precedes new configure (channel-local
// release).  No await of [[OUT0]] yet (cross-channel independence).
// CHECK-NEXT:      aiex.dma_await_task([[IN0]])
...
// Third in: await of [[IN1]] precedes new configure.
// CHECK-NEXT:      aiex.dma_await_task([[IN1]])
