...
//
// Three cases:
//   1. Violation: iter_count=4 (total DMA sends) but consumer scf.for has
//      trip count 64 — deadlock after 4 iterations.
//      The pass should emit a warning on the conduit.create.
//
//   2. Valid: iter_count=64 and scf.for trip count=64. Sends match loop.
//      No warning.
//
//   3. No iter_count — channel skipped entirely. repeat_count alone is not
//      the total send count and is not checked. No warning.
//
// The check fires when:
//   - conduit.create has iter_count=N attribute, AND
...
//
// This catches the Exp C class of deadlock: DMA fires N times (iter_count=N),
...
module {
  aie.device(npu2) {
    ...
    // -----------------------------------------------------------------------
    // CASE 1: Violation — iter_count(4) < loop trip count(64).
    ...
    // -----------------------------------------------------------------------
    // CASE 2: Valid — iter_count(64) == loop trip count(64). No warning.
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // CASE 3: No iter_count — channel skipped.
    // repeat_count alone is not the total send count; not checked.
    ...
    %core02 = aie.core(%tile02) {
      ...
      scf.for %i = %c0 to %c64 step %c1 {
        ...
        // CASE 1 violation: iter_count=4, loop=64 → warning.
        ...
        // CASE 2 valid: iter_count=64, loop=64 → no warning.
        ...
        // CASE 3 no iter_count: channel skipped → no warning.
