...
//
// Pre-fix (canon over-collapses 4 sliding-offset gets whose chain shape
// is `[true, false]`): HW dispatch hangs (XRT timeout — same failure
// mode as the canon-link smoke and the puts-with-await smoke).
// Post-fix: dispatch completes byte-equivalent to the reference
// computed below.
//
// Note: this fixture also exercises the LEGITIMATE puts-side collapse
// (4 free-only puts on @in collapse to 1 + dma_repeat = 3); a regression
// that broke that LEGITIMATE collapse path would also surface here as
// wrong bytes (e.g. only 1 input replica written instead of 4).
...
int main(int argc, const char *argv[]) {
  cxxopts::Options options("conduit_canon_no_collapse_on_gets_with_await");
