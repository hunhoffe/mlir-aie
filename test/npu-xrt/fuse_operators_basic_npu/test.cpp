...
//===----------------------------------------------------------------------===//
//
// XRT host harness for `--conduit-fuse-operators` 1:1 spatial fusion HW
// smoke (basic).  E2E companion to lit-only pin
...
// Outputs: one 64-element bf16 buffer, out[j] = (in[j] + 1.0) * 2.0.
//
...
// the merged-device ELF deadlock/misroute class — Bug A/B-style failures
// that compile cleanly but fail mid-run on hardware, and #99 trim-after-
// merge corruption that surfaces as all-zero output across dispatches.
// Per-invocation verification (vs. final-only verification) localizes
// when the failure first surfaces.
//
...
int main(int argc, const char *argv[]) {
  cxxopts::Options options("fuse_operators_basic_npu");
  ...
  // arg 3 / 4 = first two runtime-sequence args of the merged device, in
  // source-order: ext_in_add, ext_out_mul.  After --conduit-fuse-operators
  // pairs and erases the intermediate channel pair (inter_add /
  // consume_add), only the external-facing input/output args survive.
  run.set_arg(3, bo_in);
  run.set_arg(4, bo_out);

  // Reference: out[j] = (in[j] + 1.0) * 2.0, all bf16-exact ints.
