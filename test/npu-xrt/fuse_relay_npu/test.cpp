...
//===----------------------------------------------------------------------===//
//
// XRT host harness for `--conduit-fuse-relay` (gather→scatter relay
// folding) HW smoke.  E2E companion to lit-only pin
...
// Outputs: one 64-element bf16 buffer, out[j] = in[j] + 1.0.
//
// Multi-invocation: the harness dispatches the merged kernel 4 times
// (mirroring sibling fuse_core_bodies_npu/test.cpp's NUM_INVOCATIONS=4)
// so the smoke catches the relay-elimination second-invocation lock-
// state-leftover failure class — relay-elimination on the input path
...
// dispatch 2 deadlocks waiting on a stale lock.  Single dispatch alone
// would silently hide that failure mode.
//
// Max output value = 15 + 1 = 16 < 256, well within bf16-exact integer
...
int main(int argc, const char *argv[]) {
  cxxopts::Options options("fuse_relay_npu");
  ...
  // arg 3 / 4 = first two runtime-sequence args of the device, in
  // source-order: in_shim, out_shim.  The intermediate (in_compute)
  // channel is internal — fuse-relay folds it AND the matching
  // gather/scatter pair into a single conduit.transpose, but the
  // external-facing input/output args are unchanged.
  run.set_arg(3, bo_in);
  run.set_arg(4, bo_out);

  // Reference: out[j] = in[j] + 1.0, all bf16-exact ints.
  std::vector<uint16_t> ref(IO_LEN, 0);
  for (int j = 0; j < IO_LEN; j++) {
    float a = bf16_to_float(input_vec[j]);
    ref[j] = float_to_bf16(a + 1.0f);
  }
