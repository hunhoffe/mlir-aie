...
//===----------------------------------------------------------------------===//
//
// XRT host harness for the COMPOSITION `--conduit-fuse-spatial` →
// `--conduit-fuse-channels` HW smoke.  Sibling-pair to single-pass smokes:
//   test/npu-xrt/fuse_operators_basic_npu/test.cpp
//   test/npu-xrt/fuse_channels_npu/test.cpp
// And companion to:
//   test/npu-xrt/fuse_channels_after_core_bodies_npu/test.cpp
//
...
// smoke catches the merged-device ELF deadlock/misroute class — Bug A/B-style
// failures that compile cleanly but fail mid-run on hardware, and any
...
int main(int argc, const char *argv[]) {
  cxxopts::Options options("fuse_spatial_and_channels_npu");
  ...
  // input/output args survive — same erase pattern as basic-spatial sibling,
  // just with one additional surviving output.
  run.set_arg(3, bo_in);
