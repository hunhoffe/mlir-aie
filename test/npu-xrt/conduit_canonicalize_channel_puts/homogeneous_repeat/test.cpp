...
//===----------------------------------------------------------------------===//
//
// XRT host harness for HomogeneousRepeatPattern --conduit-canonicalize-
...
// (--use-conduit triggers --conduit-canonicalize-channel-puts).  Identical
// host program for both — the byte-equivalence contract is that conduit's
// canon collapse must produce the same output bytes as the stateful
// 4-individual-BD lowering.
...
static uint16_t float_to_bf16(float f) {
  ...
  // Truncate-toward-zero is exact for integers in [0, 256) (their float32
  // representation has zero in the low 16 mantissa bits).
  return static_cast<uint16_t>(bits >> 16);
}

int main(int argc, const char *argv[]) {
  cxxopts::Options options("conduit_canon_homogeneous_repeat");
  ...
  // Reference: 4 identical replicas of input.
