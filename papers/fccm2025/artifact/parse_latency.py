...
def parse_latency(collection_dir: str, output_file: str):
    ...
    '''
    avg_iron = statistics.mean(iron_latencies)
    avg_iron_ext = statistics.mean(iron_ext_latencies)
    avg_diff = abs(avg_iron - avg_iron_ext) / ((avg_iron + avg_iron_ext) / 2)
    print(f"Percent diff average across corpus is: {avg_diff}")
    '''
    print(
